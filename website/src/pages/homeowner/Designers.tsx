...
export default function Designers() {
  ...
  useEffect(() => {
    getProfessionals('interior_designer')
            ...
            <div key={pro.id} className="pro-card">
              <div className="pro-card-header">
                <div className="pro-avatar" style={{background:'linear-gradient(135deg,#EC4899,#BE185D)'}}>
                  {(pro.name || 'D').charAt(0).toUpperCase()}
                </div>
                <div style={{flex:1}}>
              ...
              {pro.specializations && pro.specializations.length > 0 && (
                <div className="tags">
                  {pro.specializations.slice(0,3).map(s => <span key={s} className="tag" style={{background:'rgba(236,72,153,0.12)',color:'#F472B6',borderColor:'rgba(236,72,153,0.2)'}}>{s}</span>)}
                </div>
              )}
              <div className="pro-footer">
                <div>
                  <div className="pro-price" style={{color:'#EC4899'}}>{pro.price_range || 'Get Quote'}</div>
                </div>
                <div className="pro-actions">
                  <button className="btn btn-sm" style={{background:'linear-gradient(135deg,#EC4899,#BE185D)',color:'white'}} onClick={() => navigate(`/chat/${pro.id}`)}>💬 Chat</button>
