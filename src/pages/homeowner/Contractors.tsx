...
export default function Contractors() {
  ...
  useEffect(() => {
    getProfessionals('contractor').then(r => setPros(r.data)).catch(() => toast('Failed to load', 'error')).finally(() => setLoading(false))
  }, [])
          ...
          {filtered.map(pro => (
            <div key={pro._id} className="pro-card">
              ...
              {pro.profile.specializations?.length > 0 && (
                <div className="tags">
                  {pro.profile.specializations.slice(0,3).map(s => <span key={s} className="tag" style={{background:'rgba(194,65,12,0.12)',color:'#F97316',borderColor:'rgba(194,65,12,0.2)'}}>{s}</span>)}
                </div>
              )}
              <div className="pro-footer">
                <div>
                  <div className="pro-price" style={{color:'var(--accent)'}}>{pro.profile.price_range || '₹1400-1800/sqft'}</div>
